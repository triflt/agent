...
class OpenAIBot:
    def __init__(self):
        ...
        try:
            ...
            self.logger.error(f"Failed to initialize RAG engine: {str(e)}", exc_info=True)
            raise
    ...
    def generate_response(self, query: str, request_id: int) -> PredictionResponse:
        ...
        try:
            ...
            context, urls = self.rag_engine.get_relevant_context(query, num_chunks=4)
            self.logger.info(
            ...
            self.logger.info(f"Received structured response: {parsed_response.model_dump_json(indent=2)}")
